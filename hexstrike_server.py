...
class _SafeStreamHandler(logging.StreamHandler):
    """StreamHandler that never crashes on UnicodeEncodeError (e.g. Windows cp1252)."""
...
if platform.system() == "Windows":
    _DEFAULT_WORDLIST_DIR = str(
        Path(os.environ.get("USERPROFILE", str(Path.home()))) / "wordlists"
    )
else:
    ...
    def _execute_command_internal(
        ...
        try:
            ...
            if resource_usage["cpu_percent"] > self.resource_thresholds["cpu_high"]:
                # Add nice priority for CPU-intensive commands (Linux/macOS only)
                if os.name != "nt" and not command.startswith("nice"):
...
class PythonEnvironmentManager:
    ...
    def __init__(self, base_dir: str = None):
        self.base_dir = (
            Path(base_dir)
            if base_dir
            else Path(tempfile.gettempdir()) / "hexstrike_envs"
        )
    ...
    def install_package(self, env_name: str, package: str) -> bool:
        ...
        pip_path = env_path / scripts_dir / "pip"
    ...
    def get_python_path(self, env_name: str) -> str:
        ...
        env_path = self.create_venv(env_name)
        scripts_dir = "Scripts" if os.name == "nt" else "bin"
        return str(env_path / scripts_dir / "python")
...
class FileOperationsManager:
    ...
    def __init__(self, base_dir: str = None):
        self.base_dir = (
            Path(base_dir)
            if base_dir
            else Path(tempfile.gettempdir()) / "hexstrike_files"
        )
        self.base_dir.mkdir(exist_ok=True)
...
def health_check():
    ...
    now = time.time()
    if (
        _HEALTH_TOOL_CACHE["tools_status"] is not None
        and now - _HEALTH_TOOL_CACHE["timestamp"] < _HEALTH_TOOL_CACHE_TTL
    ):
        ...
        for tool in all_tools:
            try:
                detect_cmd = "where" if os.name == "nt" else "which"
                result = execute_command(f"{detect_cmd} {tool}", use_cache=True)
...
class TorClient:
    ...
    def __init__(
        self,
        socks_proxy: str = TOR_SOCKS_PROXY,
    ...
    def fetch(
        ...
        try:
            ...
            return {
                "success": True,
                "url": url,
                "is_onion": url.lower().split("/")[2].endswith(".onion")
                if "//" in url
                else False,
        ...
        "breach_credential": [
            {
                ...
                "selectors": ["email"],
                "access": "free/api-key",
                "note": "breach membership for an email/domain",
            },
            {
                "name": "DeHashed",
                "url": "https://dehashed.com/",
                "selectors": ["email", "phone", "name", "username", "id"],
                "access": "paid",
                "note": "indexed breach records — broad selector pivot",
            },
            {
                "name": "Intelligence X",
                "url": "https://intelx.io/",
                "selectors": ["email", "phone", "domain", "name"],
                "access": "api-key",
                "note": "leaks, pastes, darknet selectors",
            },
            {
                "name": "LeakCheck",
                "url": "https://leakcheck.io/",
                "selectors": ["email", "phone", "username"],
                "access": "paid",
                "note": "breach lookup by selector",
            },
            {
                "name": "Snusbase",
                "url": "https://snusbase.com/",
                "selectors": ["email", "phone", "name", "username"],
                "access": "paid",
                "note": "fast breach search",
            },
            {
                "name": "BreachDirectory",
                "url": "https://breachdirectory.org/",
                "selectors": ["email", "username"],
                "access": "free/api-key",
                "note": "free-tier breach check",
            },
            {
                "name": "h8mail",
                ...
                "selectors": ["email"],
                "access": "tool",
                "note": "email -> breach pivot, chains many APIs",
            },
        ],
        "email": [
            {
                ...
                "access": "free/paid",
                "note": "email/phone -> linked Google/account services",
            },
            {
                "name": "holehe",
                ...
                "selectors": ["email"],
                "access": "tool",
                "note": "checks 120+ sites for account existence",
            },
            {
                "name": "EmailRep",
                ...
                "selectors": ["email"],
                "access": "free/api-key",
                "note": "reputation + profile signals",
            },
            {
                "name": "Hunter.io",
                "url": "https://hunter.io/",
                "selectors": ["domain", "email"],
                "access": "api-key",
                "note": "email pattern + verified addresses per domain",
            },
        ],
        "phone": [
            {
                ...
                "selectors": ["phone"],
                "access": "tool",
                "note": "phone footprint + carrier + OSINT scan",
            },
            {
                "name": "NumVerify",
                ...
                "selectors": ["phone"],
                "access": "api-key",
                "note": "validation, carrier, line type, country",
            },
            {
                "name": "Truecaller",
                ...
                "selectors": ["phone"],
                "access": "gated",
                "note": "name <-> phone (login required)",
            },
        ],
        "name_identity": [
            {
                ...
                "selectors": ["name", "phone", "email"],
                "access": "free",
                "note": "name -> address/phone (US-centric)",
            },
            {
                "name": "Pipl-style aggregators",
                "url": "https://pipl.com/",
                "selectors": ["name", "email", "phone"],
                "access": "paid",
                "note": "identity resolution (enterprise)",
            },
            {
                "name": "WhitePages / FastPeopleSearch",
                "url": "https://www.fastpeoplesearch.com/",
                "selectors": ["name", "phone"],
                "access": "free",
                "note": "US people search",
            },
        ],
        "username_social": [
            {
                ...
                "selectors": ["username"],
                "access": "tool",
                "note": "hunt username across 400+ sites",
            },
            {
                "name": "WhatsMyName",
            ...
            },
            {
                ...
                "selectors": ["username"],
                "access": "tool",
                "note": "username -> profiles + extracted PII",
            },
        ],
        "domain_infra": [
            {
                ...
                "selectors": ["domain"],
                "access": "free",
                "note": "certificate transparency hostnames",
            },
            {
                "name": "SecurityTrails",
        ...
        "search_engines": [
            {
                ...
                "selectors": ["name", "email", "phone", "id"],
                "access": "free",
                "note": "site:/filetype:/intext: operators",
            },
            {
                "name": "Yandex",
                ...
                "selectors": ["name"],
                "access": "free",
                "note": "strong reverse-image + non-EN coverage",
            },
            {
                "name": "DuckDuckGo",
                ...
                "access": "free",
                "note": "less personalized indexing",
            },
        ],
    },
    "onion": {
        "search_index": [
            {
                ...
                "selectors": ["name", "email", "domain"],
                "access": "free",
                "note": "filtered onion search (clearnet mirror too)",
            },
            {
                "name": "Torch",
                "url": "",
            ...
            },
            {
                ...
                "selectors": ["domain"],
                "access": "free",
                "note": "verified onion mirror directory",
            },
        ],
        "leak_monitoring": [
            {
                ...
                "selectors": ["name", "domain"],
                "access": "free",
                "note": "aggregates ransomware leak sites (DLS) — defensive monitoring",
            },
            {
                "name": "RansomWatch",
                ...
                "selectors": ["name"],
                "access": "free",
                "note": "tracks DLS victim postings",
            },
        ],
        "paste": [
            {
                "name": "psbdmp",
                "url": "https://psbdmp.ws/",
                "selectors": ["email", "domain"],
                "access": "free/api",
                "note": "deleted/archived paste search",
            },
        ],
    },
    "brazil": {
        "identity_cpf": [
            {
                "name": "Receita Federal - Consulta CPF",
                "url": "https://servicos.receita.fazenda.gov.br/Servicos/CPF/ConsultaSituacao/ConsultaPublica.asp",
                "selectors": ["cpf"],
                "access": "official",
                "note": "official CPF status — requires CPF + birthdate + captcha. Validation only, not enrichment",
            },
            {
                "name": "TSE - Consulta Situação Eleitoral",
                "url": "https://www.tse.jus.br/eleitor/situacao-eleitoral",
                "selectors": ["cpf", "name"],
                "access": "official",
                "note": "voter registration status (official)",
            },
            {
                ...
                "selectors": ["name", "cpf"],
                "access": "official",
                "note": "public servants, benefits, sanctions (partial CPF masked)",
            },
            {
                "name": "Escavador",
                ...
                "selectors": ["name", "cpf"],
                "access": "free/paid",
                "note": "name -> lawsuits, professional history, public records",
            },
            {
                "name": "JusBrasil",
                ...
                "selectors": ["name", "cpf"],
                "access": "free/paid",
                "note": "legal records, processes mentioning a person",
            },
        ],
        "company_cnpj": [
            {
                ...
                "selectors": ["cnpj"],
                "access": "free/api",
                "note": "CNPJ -> company registration data",
            },
            {
                "name": "BrasilAPI CNPJ",
                ...
                "selectors": ["cnpj"],
                "access": "free-api",
                "note": "official-source CNPJ lookup",
            },
            {
                "name": "CNPJ.biz / casadosdados",
                "url": "https://casadosdados.com.br/",
                "selectors": ["cnpj", "name"],
                "access": "free",
                "note": "company search + partners (sócios)",
            },
        ],
        "phone_br": [
            {
                ...
                "selectors": ["phone"],
                "access": "free",
                "note": "caller reputation, spam reports (BR)",
            },
            {
                "name": "Qual Número",
                "url": "https://www.qualnumero.com.br/",
                "selectors": ["phone"],
                "access": "free",
                "note": "carrier/region by DDD + number",
            },
            {
                "name": "Anatel - operadora/portabilidade",
                "url": "https://www.anatel.gov.br/",
                "selectors": ["phone"],
                "access": "official",
                "note": "number portability / carrier reference",
            },
        ],
        "legal_court_br": [
            {
                "name": "TJ e-SAJ (estaduais)",
                "url": "https://esaj.tjsp.jus.br/",
                "selectors": ["name", "cpf"],
                "access": "official",
                "note": "state court process consult by name/CPF",
            },
            {
                "name": "PJe / CNJ",
                ...
                "selectors": ["name"],
                "access": "official",
                "note": "unified judicial process lookup",
            },
        ],
        "breach_context_br": [
            {
                "name": "Intelligence X (BR leaks)",
                "url": "https://intelx.io/",
                "selectors": ["cpf", "email", "phone", "name"],
                "access": "api-key",
                "note": "indexes major BR leaks (e.g. 2021 mega-leak) — for breach-exposure assessment",
            },
            {
                "name": "DeHashed (BR records)",
                "url": "https://dehashed.com/",
                "selectors": ["cpf", "email", "phone", "name"],
                "access": "paid",
                "note": "selector pivot across BR breach data",
            },
        ],
    },
}
...
WORDLIST_REGISTRY = {
    "raft-large-directories": {
        ...
        "use": "directory/file brute force (ffuf/gobuster)",
    ...
    "rockyou": {
        "category": "passwords",
        "use": "password cracking / spraying (hashcat/john)",
    ...
    "subdomains-top1million-110000": {
        "category": "dns",
        "use": "subdomain brute force (ffuf/gobuster/puredns)",
    ...
    "n0kovo_subdomains": {
        "category": "dns",
        "use": "high-signal subdomain brute force",
        "seclists_path": None,
        "kali_paths": [],
        ...
        "filename": "n0kovo_subdomains_small.txt",
        "large": False,
    },
    "api-endpoints": {
        "category": "web-content",
        "use": "API path discovery",
    ...
    "LFI-Jhaddix": {
        "category": "fuzzing",
        "use": "local file inclusion / path traversal fuzzing",
    ...
    "crackstation": {
        "category": "passwords",
        "use": "offline hash cracking (human-only list)",
        "seclists_path": None,
        "kali_paths": [],
    ...
    "onelistforall": {
        "category": "web-content",
        "use": "all-in-one content discovery (six2dez)",
        "seclists_path": None,
        "kali_paths": [],
        ...
        "filename": "onelistforallmicro.txt",
        "large": False,
    },
}
...
class WordlistManager:
    ...
    def detect_environment(self) -> Dict[str, Any]:
        ...
        info = {
            ...
            "distro": "",
            "seclists_base": None,
            "wordlist_base": str(self.base_dir),
        }
        ...
        if osrel.exists():
            try:
                ...
                pass
        for cand in self.SECLISTS_BASES + [
            str(self.base_dir / "SecLists"),
            str(self.base_dir / "seclists"),
        ]:
            if Path(cand).is_dir():
        ...
        for c in self._candidates(name, entry):
            ...
            if Path(c + ".gz").exists():
                return {
                    ...
                    "found": True,
                    "path": c + ".gz",
                    "source": "local-compressed",
                    "note": "gunzip before use",
                    "category": entry["category"],
                }
        return {
            "name": name,
    ...
    def download(
        self, name: str, allow_large: bool = False, use_large_variant: bool = False
    ) -> Dict[str, Any]:
        entry = WORDLIST_REGISTRY.get(name)
        if not entry:
        ...
        url = (
            entry.get("url_large")
            if (use_large_variant and entry.get("url_large"))
            else entry.get("url")
        )
        if not url:
        ...
        if entry.get("large") and not allow_large:
            return {
                ...
                "success": False,
                "skipped": True,
                "reason": "large file — pass allow_large=true to download",
                "download_url": url,
            }
        try:
    ...
    def _install_hint(self) -> str:
        ...
        if self.env["is_windows"]:
            return (
                f'Windows: `git clone https://github.com/danielmiessler/SecLists "{self.base_dir}\\SecLists"` '
                f"or POST /api/wordlists/setup with download=true to pull individual lists into {self.base_dir}."
            )
        return (
            f"Linux: `git clone https://github.com/danielmiessler/SecLists {self.base_dir}/SecLists` "
    ...
    def cewl_command(
        ...
        cmd = f"cewl -d {depth} -m {min_length} -w {out} {extra} {url}".replace(
            "  ", " "
        ).strip()
...
class CyberRangeLabManager:
    ...
    # ----------------------------------------------------------------------
    def build_pentest_lab(
        self,
        scenario: str = "web-app-gauntlet",
        tier: str = "",
        ...
        scenario_def = self.lab_scenarios.get(
            scenario, self.lab_scenarios["web-app-gauntlet"]
        )
        ...
        blueprint = {
                ...
                "segments": [
                    {
                        "name": "dmz",
                        "cidr": "10.10.10.0/24",
                        "hosts": ["web-edge", "reverse-proxy"],
                    },
                    {
                        "name": "internal",
                        "cidr": "10.10.20.0/24",
                        "hosts": ["app-server", "db-server"],
                    },
                    {
                        "name": "mgmt",
                        "cidr": "10.10.99.0/24",
                        "hosts": ["jump-host", "siem-collector"],
                    },
                ],
                "isolation": "host-only / no egress to internet (range firewall enforced)",
                "pivot_paths": [
                    "dmz -> internal via app-server",
                    "internal -> mgmt via stolen creds",
                ],
            },
            ...
            "lab_phases": [
                {
                    ...
                    "tools": ["nmap", "naabu", "httpx", "katana"],
                    "deliverable": "host+service inventory",
                    "points": 100,
                },
                {
                    "phase": "foothold",
                    "objective": "Achieve initial access",
                    "tools": ["nuclei", "sqlmap", "ffuf", "burpsuite"],
                    "deliverable": "first shell / valid creds",
                    "points": 250,
                },
                {
                    "phase": "privilege-escalation",
                    "objective": "Escalate locally",
                    "tools": ["linpeas", "winpeas", "GTFOBins"],
                    "deliverable": "root/SYSTEM",
                    "points": 250,
                },
                {
                    "phase": "lateral-movement",
                    "objective": "Pivot across segments",
                    "tools": ["chisel", "ligolo-ng", "crackmapexec"],
                    "deliverable": "second host owned",
                    "points": 200,
                },
                {
                    "phase": "objective-capture",
                    "objective": "Reach crown-jewel flag",
                    "tools": ["scenario-specific"],
                    "deliverable": "final flag + report",
                    "points": 200,
                },
            ],
            "learning_objectives": [
            ...
            "graduated_hints": [
                {
                    "cost": 25,
                    "hint": "Re-check service banners for version-specific CVEs",
                },
                {
                    "cost": 50,
                    "hint": "An exposed parameter trusts user input without validation",
                },
                {
                    "cost": 75,
                    "hint": "Reused credentials bridge the dmz->internal boundary",
                },
            ],
            "instructor_notes": [
    ...
    def _target_specs_for(self, vulns: List[str]) -> List[Dict[str, Any]]:
        ...
        catalog = {
            "sqli": {
                ...
                "weakness": "unparameterized query in /search",
                "flag_location": "users.secret column",
                "intended_tool": "sqlmap",
            },
            "xss": {
                "base_image": "node:20-alpine",
                "weakness": "reflected name param, no output encoding",
                "flag_location": "admin session cookie",
                "intended_tool": "burpsuite",
            },
            "ssrf": {
                "base_image": "python:3.12",
                "weakness": "url-fetch feature trusts user host",
                "flag_location": "cloud metadata / internal admin",
                "intended_tool": "ffuf+collaborator",
            },
            "idor": {
                "base_image": "ruby:3.3",
                "weakness": "object id not authz-checked",
                "flag_location": "other tenant invoice",
                "intended_tool": "burpsuite",
            },
            "file-upload": {
                "base_image": "php:8-apache",
                "weakness": "extension allowlist bypassable",
                "flag_location": "webshell RCE -> /root/flag",
                "intended_tool": "manual+wfuzz",
            },
            "auth-bypass": {
                "base_image": "node:20-alpine",
                "weakness": "JWT alg:none accepted",
                "flag_location": "admin panel",
                "intended_tool": "jwt_tool",
            },
            "kerberoast": {
                "base_image": "windows-server-ad",
                "weakness": "SPN account weak password",
                "flag_location": "service account hash",
                "intended_tool": "impacket-GetUserSPNs",
            },
            "buffer-overflow": {
                "base_image": "ubuntu:22.04 (no-canary, no-PIE)",
                "weakness": "gets() in setuid binary",
                "flag_location": "/root/flag via ret2libc",
                "intended_tool": "pwntools+gdb-pwndbg",
            },
        }
        specs = []
        for v in vulns:
            spec = catalog.get(
                v,
                {
                    ...
                    "weakness": f"deliberate {v} weakness",
                    "flag_location": "scenario flag",
                    "intended_tool": "operator choice",
                },
            )
    ...
    def _war_room_scoring(self) -> Dict[str, Any]:
        return {
            ...
            "red_team_points": {
                "first_blood": 500,
                "flag_capture": 250,
                "persistence_held_per_tick": 50,
            },
            "blue_team_points": {
                "detection_within_5min": 300,
                "containment": 250,
                "clean_eviction": 200,
    ...
    def _wordlists_for_focus(self, focus: str) -> Dict[str, Any]:
        ...
        names = focus_map.get(
            focus,
            ["raft-large-directories", "subdomains-top1million-110000", "rockyou"],
        )
    ...
    # ----------------------------------------------------------------------
    def build_osint_investigation(
        self,
        target: str,
        depth: str = "surface",
        investigation_type: str = "organization",
    ) -> Dict[str, Any]:
                ...
                "phases": [
                    {
                        ...
                        "tools": [
                            ...
                            {
                                "tool": "crt.sh / certificate_transparency",
                                "purpose": "cert-derived hostnames",
                            },
                        ],
                    },
                    {
                        "name": "search-footprint",
                        "tools": [
                            {
                                "tool": "google-dorking",
                                "purpose": "indexed sensitive files/paths",
                            },
                            {
                                "tool": "wayback/gau",
                                "purpose": "historical URLs + endpoints",
                            },
                            {
                                "tool": "shodan / censys",
                                "purpose": "exposed services + banners",
                            },
                        ],
                    },
                    {
                        ...
                        "tools": [
                            ...
                            {
                                "tool": "sherlock",
                                "purpose": "username across platforms",
                            },
                            {
                                "tool": "linkedin/social_mapper",
                                "purpose": "org chart + roles",
                            },
                        ],
                    },
                    {
                        "name": "breach-exposure-surface",
                        "tools": [
                            {
                                "tool": "haveibeenpwned",
                                "purpose": "known breach membership",
                            },
                            {
                                "tool": "hunter_io",
                                "purpose": "email pattern + verified addresses",
                            },
                        ],
                    },
                ],
            ...
            plan["layers"].append(
                {
                    ...
                    "phases": [
                        {
                            "name": "paste-and-leak-monitoring",
                            "sources": [
                                "pastebin/ghostbin scrapers (psbdmp)",
                                "github/gitlab dorking for secrets (trufflehog, gitleaks)",
                        ...
                        },
                        {
                            ...
                            "sources": [
                                ...
                                "intelx.io selectors",
                                "h8mail email->breach pivot",
                            ],
                            "purpose": "exposed credentials for the org's selectors",
                        },
                        {
                            "name": "deep-document-intel",
                            "sources": [
                                "SEC/EDGAR, court records, procurement portals",
                                "academic + patent DBs",
                            ],
            ...
            plan["layers"].append(
                {
                    ...
                    "transport": {
                        ...
                        "rules": [
                            ...
                            "no engagement with criminal services",
                            "no purchases",
                            "preserve evidence, report per IR policy",
                        ],
                    },
                    "phases": [
                        {
                        ...
                        {
                            "name": "ioc-extraction",
                            "sources": ["actor TTP write-ups", "paste-of-paste dumps"],
        ...
        plan["estimated_time_minutes"] = {"surface": 120, "deep": 300, "full": 540}.get(
            depth, 120
        )
        return plan
    ...
    # ----------------------------------------------------------------------
    def build_selector_investigation(
        self,
        selector: str,
        selector_type: str = "email",
        country: str = "global",
        depth: str = "surface",
    ) -> Dict[str, Any]:
        ...
        plan = {
            ...
            "pivots": {
                "email": [
                    "-> breach records -> reused passwords",
                    "-> linked accounts (holehe/epieos)",
                    "-> username -> social",
                ],
                "phone": [
                    "-> carrier/region",
                ...
                "name": [
                    ...
                    "-> email pattern via employer domain",
                ],
                "id": [
                    "-> official validation",
                    "-> court/transparency records",
    ...
    # ----------------------------------------------------------------------
    def build_payload_lab(
        self,
        payload_class: str = "web",
        target_context: str = "",
        evasion_focus: bool = False,
    ) -> Dict[str, Any]:
        ...
        modules = {
            "web": {
                ...
                "verification": [
                    "canary callback received",
                    "deterministic time-delay observed",
                    "OOB interaction logged",
                ],
            },
            "shellcode": {
                ...
                "verification": [
                    "controlled EIP/RIP",
                    "clean shellcode execution in debugger",
                    "no crash on bad-char",
                ],
            },
            "implant": {
                ...
                "techniques": [
                    {
                        ...
                        "defeats": "static string match",
                        "detect_with": "entropy + behavior",
                    },
                    {
                        "name": "polymorphism",
                        "defeats": "hash/sig",
                        "detect_with": "behavioral/EDR telemetry",
                    },
                    {
                        "name": "living-off-the-land",
                        "defeats": "binary allowlist",
                        "detect_with": "command-line + parent-child anomaly",
                    },
                    {
                        "name": "sleep/jitter + domain-fronting",
                        "defeats": "naive netflow",
                        "detect_with": "JA3/JA4 + beacon analysis",
                    },
                ],
    ...
    # ----------------------------------------------------------------------
    def build_malware_lab(
        self, sample_family: str = "loader", track: str = "both"
    ) -> Dict[str, Any]:
            ...
            lab["offense_track"] = {
                ...
                "family_blueprint": {
                    ...
                    "ransomware-sim": [
                        ...
                        "REVERSIBLE marker-encrypt of lab data only",
                        "drop note",
                    ],
                    "infostealer-sim": [
                        "locate decoy creds",
                        "stage to lab collector",
                        "exfil over lab channel",
                    ],
        ...
        if track in ("defense", "both"):
            lab["defense_track"] = {
                ...
                "yara_authoring": {
                    ...
                    "quality_gate": [
                        "no false positives on goodware corpus",
                        "survives recompile",
                        "documents rationale",
                    ],
                },
...
def build_pentest_lab_endpoint():
    ...
    try:
        ...
        logger.info(
            f"🥋 Building pentest lab | scenario={scenario} tier={tier or 'default'} mode={team_mode}"
        )
...
def build_osint_investigation_endpoint():
    ...
    try:
        ...
        plan = cyber_range_manager.build_osint_investigation(
            target, depth, investigation_type
        )
        return jsonify(
            {
                "success": True,
...
def build_payload_lab_endpoint():
    ...
    try:
        ...
        logger.info(
            f"🧪 Building payload lab | class={payload_class} evasion={evasion_focus}"
        )
        module = cyber_range_manager.build_payload_lab(
            payload_class, target_context, evasion_focus
        )
        return jsonify(
            {
                "success": True,
        ...
        return jsonify(
            {
                "success": True,
                "environment": env,
                "install_hint": wordlist_manager._install_hint(),
                "timestamp": datetime.now().isoformat(),
            }
        )
    except Exception as e:
...
def wordlists_registry():
    ...
    try:
        return jsonify(
            {
                "success": True,
                "count": len(WORDLIST_REGISTRY),
                "registry": WORDLIST_REGISTRY,
                "timestamp": datetime.now().isoformat(),
            }
        )
    except Exception as e:
...
def wordlists_setup():
    ...
    try:
        ...
        logger.info(
            f"📚 Wordlist setup | download={download} allow_large={allow_large} names={names or 'ALL'}"
        )
        result = wordlist_manager.setup(
            names=names, download=download, allow_large=allow_large
        )
        return jsonify(
...
def wordlists_cewl():
    ...
    try:
        ...
        cmd = wordlist_manager.cewl_command(
            ...
            min_length=int(data.get("min_length", 5)),
            output=data.get("output", ""),
            extra=data.get("extra", ""),
        )
        return jsonify(
            {"success": True, "cewl": cmd, "timestamp": datetime.now().isoformat()}
        )
    except Exception as e:
...
def osint_sources():
    ...
    try:
        ...
        total = sum(
            len(entries) for groups in registry.values() for entries in groups.values()
        )
        return jsonify(
            {
                "success": True,
...
def osint_selector_search():
    ...
    try:
        ...
        logger.info(
            f"🔎 Selector investigation | type={selector_type} country={country} depth={depth}"
        )
        plan = cyber_range_manager.build_selector_investigation(
            selector, selector_type, country, depth
        )
        return jsonify(
            {
                "success": True,
...
def osint_tor_fetch():
    ...
    try:
        ...
        result = tor_client.fetch(
            url, method=method, timeout=timeout, extra_headers=extra_headers
        )
        return jsonify(
            {
                "success": result.get("success", False),
